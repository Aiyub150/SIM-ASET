...
Neither the GD library nor the Imagick extension is installed!" (Fixed)
...
- Option pilih device scanner tidak mendeteksi kamera, seharusnya web dapat mendeteksi kamera pada semua device entah kamera webcam atau kamera belakang handphone sehingga di bagian option akan muncul "Keyboard input/scanner" dengan beberapa opsi device kamera sesuai dengan device yang mengaksesnya (Fixed)

- Search bar tidak perlu dibuat input yang berbeda dengan option menu untuk barang maupun kategori barang, seharusnya search bar ada di dalam option input (Fixed)